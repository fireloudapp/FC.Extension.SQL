...
namespace FC.Extension.SQL.MySQL
{
    ...
    public class MySQLDataAccess<TModel> : IBaseAccess<TModel> where TModel : class
    {
        string _conString = string.Empty;
        BaseTrace _baseTrace = null;
        MySqlCompiler _mysqlompiler;
        #region Constructor
        public MySQLDataAccess(string connectionString, BaseTrace baseTrace = null)
        {
            _conString = connectionString;
            _baseTrace = baseTrace;
            RepoDb.MySqlBootstrap.Initialize();
            _mysqlompiler = new MySqlCompiler();
        }
        #endregion
        ...
        public async Task<IEnumerable<TModel>> ExecuteQuery(Query query)
        {
            ...
            try
            {
                SqlResult sqlResult = _mysqlompiler.Compile(query);
                #region DEBUG
